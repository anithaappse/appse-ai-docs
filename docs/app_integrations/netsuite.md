...
| Consumer Key     | Consumer key from your Netsuite integration record            |
| Consumer Secret  | Consumer secret from your Netsuite integration record         |
| Token ID         | Token ID generated for your Netsuite user and integration     |
| Token Secret     | Token secret generated for your Netsuite user and integration |
...
#### 3. Create or Verify an Integration Record

- In Netsuite, go to `Setup` -> `Integration` -> `Manage Integrations` -> `New`.
- Enter an integration name (for example: `appse ai integration`).
- Enable token-based authentication if prompted.
...
#### 4. Generate Access Token Credentials

- Go to `Setup` -> `Users/Roles` -> `Access Tokens` -> `New`.
- Select the integration record and user/role for API access.
- Save the token.
...
> `Important`: Consumer Secret and Token Secret may be shown only once. Copy and store them securely before closing the page.

#### 5. Add the Fields to appse ai
...
- If it fails, verify all copied keys/tokens and ensure the user role has required API permissions.
